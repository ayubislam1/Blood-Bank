...
const Navbar = () => {
	...
	return (
		<motion.div initial={{y:-250}} animate={{y:0}}  transition={{delay:1,type:"spring",stiffness:50}} className="flex h-20 w-full  items-center px-4 md:px-6 border-b bg-gradient-to-r  -mb-2">
			<Sheet>
				<SheetTrigger asChild>
					<Button
						variant="outline"
						size="icon"
						className="lg:hidden hover:bg-opacity-80 transition-all "
					>
				...
				<SheetContent
					side="left"
					className="bg-white shadow-lg rounded-lg transition-transform ease-in-out"
				>
			...
			<motion.div  className="ml-auto hidden lg:flex gap-6">
				...
				<Link
					to="/blog"
					className={`group inline-flex h-9 px-4 py-2 rounded-md text-sm font-medium text-red-500 hover:bg-red-500 hover:text-white transition-colors ${
						isActive("/blog") ? "bg-red-500 text-white" : ""
					}`}
				>
					Blog
				</Link>
				<Link
					to="/donation-requests"
					className={`group inline-flex h-9 px-4 py-2 rounded-md text-sm font-medium text-red-500 hover:bg-red-500 hover:text-white transition-colors ${
						isActive("/donation-requests") ? "bg-red-500 text-white" : ""
					}`}
				>
					Donation Requests
				</Link>
				{user ? (
					<>
						<Link
							to="/funding"
							className={`group inline-flex h-9 px-4 py-2 rounded-md text-sm font-medium text-red-500 hover:bg-red-500 hover:text-white transition-colors ${
								isActive("/funding") ? "bg-red-500 text-white" : ""
							}`}
						>
							Funding
						</Link>
					...
					<>
						...
						<Link
							to="/register"
							className={`group inline-flex h-9 px-4 py-2 rounded-md text-sm font-medium text-red-500 hover:bg-red-500 hover:text-white transition-colors ${
								isActive("/register") ? "bg-red-500 text-white" : ""
							}`}
						>
							Register
						</Link>
